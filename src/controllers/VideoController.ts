...
const publishAVideo = asyncHandler(async (req: Request, res: Response) => {
	...
	if (!req.files?.videoFile || !req.files?.thumbnail) {
		return new ApiError(400, "Video file and thumbnail both are required");
	}
...
const updateVideo = asyncHandler(async (req: Request, res: Response) => {
	...
	if (!updatedVideo) {
		return new ApiError(500, "Something went wrong while updating video");
	}
...
const deleteVideo = asyncHandler(async (req: Request, res: Response) => {
	...
	if (!deletedVideo) {
		return new ApiError(500, "Something went wrong while deleting video");
	}
