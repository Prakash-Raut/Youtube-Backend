...
const addComment = asyncHandler(async (req: Request, res: Response) => {
	...
	if (!result.success) {
		return new ApiError(400, "Validation Error", result.error);
	}
	...
	if (!comment) {
		return new ApiError(400, "Something went wrong while adding comment");
	}
...
const updateComment = asyncHandler(async (req: Request, res: Response) => {
	...
	if (!comment) {
		return new ApiError(400, "Something went wrong while updating comment");
	}
...
const deleteComment = asyncHandler(async (req: Request, res: Response) => {
	...
	if (!comment) {
		return new ApiError(400, "Something went wrong while deleting comment");
	}
